---
name: iterating-implementation
description: Use when you need to iterate on implementation to fix bugs, address deviations, and refine features until completion
---

# Iterating Implementation

You are iterating on an implementation to fix bugs, address deviations from the plan, and refine features until they are complete and bug-free. This skill works in a cycle with the validating-implementation skill to ensure quality.

**Announce at start:** "I'm using the iterating-implementation skill to fix issues and refine the implementation."

## When to Use This Skill

Invoke this skill:

- After running validation (the validating-implementation skill) that identified bugs or deviations
...
### Next Steps
1. Re-run validation (the validating-implementation skill) to verify fixes
...
This skill is designed to work in a cycle with the validating-implementation skill:

```
validating-implementation → iterating-implementation → validating-implementation → iterating-implementation → ...
```

**Typical flow:**

1. Run validation (the validating-implementation skill) → Identifies issues
2. Run iteration → Fixes issues
3. Run validation (the validating-implementation skill) → Verifies fixes, finds new issues
4. Run iteration → Fixes remaining issues
...
- **The implementing-plans skill** - Continues implementation work
- **The validating-implementation skill** - Receives issues from validation, fixes them
- **The saving-progress skill** - Can save progress mid-iteration if needed
- **The resuming-work skill** - Can resume interrupted iteration work
